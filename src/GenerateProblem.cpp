...
// C++ std::map is not threadsafe for writing
  ...
  Kokkos::View<local_int_t*> v_rowOffsets("MatrixRowOffsets",localNumberOfRows+1);
  ...
  Kokkos::parallel_for("CopyMatrixToCRS",Kokkos::TeamPolicy<>(number_of_teams,team_size,8),
    ...
    Kokkos::parallel_for(Kokkos::TeamThreadRange(team,first_row,last_row), [&] (const local_int_t& row) {
      double* diag = matrixDiagonal[row];
      const local_int_t row_start_setup = numberOfNonzerosPerRow * row;
      const local_int_t row_start = v_rowOffsets(row);
      const local_int_t row_length = v_rowOffsets(row+1) - row_start;
      if(diag >= matrixValues[row] &&
         diag < matrixValues[row]+row_length) {
        matrixDiagonal[row] = &v_mtxValues(row_start) + (ptrdiff_t(diag) - ptrdiff_t(matrixValues[row]))/sizeof(double);
      }
      Kokkos::parallel_for(Kokkos::ThreadVectorRange(team,row_length), [&] (const local_int_t& i){
        v_mtxValues(row_start + i) = matrix_values[row_start_setup + i];
        v_mtxIndL(row_start + i) = matrix_indicies[row_start_setup + i];
      });
    });
  });
  ...
  for (local_int_t i=0; i< localNumberOfRows; ++i) {
    mtxIndL[i] = v_mtxIndL.data() + v_rowOffsets(i);
    matrixValues[i] = v_mtxValues.data() + v_rowOffsets(i);
  }
